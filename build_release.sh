...
if test -f "$FILE"; then
	...
	if (( $revisionCount < $tmpRevisionCount )); then
		...
		echo "#define KONCLUDE_VERSION_GIT_REVISION_HASH_STRING \"$revisionTagName\"" >> $FILE		
	fi
else
...
./build_release_parameterized.sh $revisionCount $revisionTagName $revisionHashName
